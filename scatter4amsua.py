...
if __name__ == '__main__':
 ...
 #grplist = ['GsiHofX','hofx0','hofx2','hofx_y_mean_xb0','hofx_y_mean_xb1']
 #grplist = ['hofx_y_mean_xb0','hofx_y_mean_xb1']
...
#-----------------------------------------------------------------------
 ...
 #print('yslist = ', yslist)
  ...
  for n in range(len(xflist)):
    ...
    xf = '%s/%s_%s_%s_m.nc4' %(xdatadir, obstype, enlist[n], datestr)
    ydatadir = '%s/hofx.%s' %(topdir, yflist[n])
    yf = '%s/%s_%s_%s_m.nc4' %(ydatadir, obstype, enlist[n], datestr)
   ...
   #hgty = ncyf.get_var('/MetaData/heightOfSurface')
    ...
    for grpname in grplist:
      ...
      basx = ncxf.get_var_2d(grpvarname)
      basy = ncyf.get_var_2d(grpvarname)

      varx = obsx - basx
      vary = obsy - basy

      for channel in range(15):
        labelname = '%s_%s_%d' %(grpname, varname, channel)
        sp42r.set_label(xlabel, ylabel, labelname, datestr=datestr, obstype=obstype)
        sp42r.scatter_plot(varx[:, channel], vary[:, channel], labelname)
